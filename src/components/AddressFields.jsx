...
//
// UX:
//   - Inicialmente só o campo CEP é mostrado
//   - Quando ViaCEP encontra o endereço (ou usuário clica em "Preencher manualmente"),
//     revela os demais campos (rua/bairro/cidade/UF já preenchidos quando vier do ViaCEP)
//     + campos número e complemento pra usuário completar
export default function AddressFields({ value, onChange, disabled = false }) {
  ...
  // Considera os campos "revelados" se já temos dados (do ViaCEP, edição anterior,
  // ou clique manual) — esses são os critérios:
  const hasAddressData = !!(v.street || v.neighborhood || v.city || v.state)
  const revealed = manualReveal || hasAddressData
  ...
  return (
    <div className="space-y-3">
      ...
      {/* CEP */}
      <div>
        <label className="text-xs mb-1 block" style={{ color: 'var(--text-tertiary)' }}>CEP</label>
        <div className="relative" style={{ maxWidth: 220 }}>
          ...
          />
          {loading && (
            <Loader2 size={14} className="animate-spin absolute right-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-muted)' }} />
          )}
        </div>
        {cepError && (
          <div className="text-[11px] mt-1.5 flex items-center gap-2" style={{ color: '#fda4af' }}>
            <span>{cepError}</span>
            {!revealed && (
              <button
                type="button"
        ...
        {!revealed && !cepError && !loading && (
          <div className="text-[11px] mt-1.5" style={{ color: 'var(--text-muted)' }}>
            Digite seu CEP — preencho o resto pra você.{' '}
            <button
              type="button"
              onClick={() => setManualReveal(true)}
              className="underline underline-offset-2 hover:opacity-80 transition"
            ...
            >
              Ou preencho manualmente
            </button>
          </div>
        )}
      </div>

      {/* Campos restantes — só aparecem após CEP ser encontrado ou clique manual */}
      {revealed && (
        <div className="space-y-3 pt-1">
          {/* Rua + UF */}
          <div className="grid sm:grid-cols-[1fr_80px] gap-3">
            <div>
              <label className="text-xs mb-1 block" style={{ color: 'var(--text-tertiary)' }}>Rua / Logradouro</label>
              <input
                type="text"
                value={v.street || ''}
            ...
            <div>
              <label className="text-xs mb-1 block" style={{ color: 'var(--text-tertiary)' }}>UF</label>
              <input
                type="text"
                value={v.state || ''}
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              <label className="text-xs mb-1 block" style={{ color: 'var(--text-tertiary)' }}>Bairro</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs mb-1 block" style={{ color: 'var(--text-tertiary)' }}>Cidade</label>
              <input
                type="text"
                value={v.city || ''}
          ...
          {/* Número + Complemento (sempre exigem ação do usuário) */}
          <div className="grid sm:grid-cols-[160px_1fr] gap-3">
            <div>
              <label className="text-xs mb-1 block" style={{ color: 'var(--text-tertiary)' }}>Número</label>
              <input
                type="text"
                value={v.number || ''}
            ...
            <div>
              <label className="text-xs mb-1 block" style={{ color: 'var(--text-tertiary)' }}>
                Complemento <span style={{ color: 'var(--text-muted)' }}>(opcional)</span>
              </label>
              <input
                type="text"
                value={v.complement || ''}
